...
form.addEventListener('submit', (e) => {
      ...
      // Valido que el Form no esté vacío
      if (validateInput()) {
            loader.classList.toggle('active');
            // Limpio el contenedor de los resultados
            clearContainer();
            const fromDate = $('#fromDate').value;
            const toDate = $('#toDate').value;
            const rate = Number($('#rate').value);
            const userId = $('#userId').value;
            const usersAndMovies = filterMovies({ users, movies, userId, fromDate, toDate, rate });

            // Uso un setTimeout para simular la demora del Fetch
            setTimeout(() => {
                  loader.classList.toggle('active');

                  // Verifico que el array no este vacío
                  if (usersAndMovies.length === 0) {
                       // si esta vaío agrego un mensaje por defecto
                        resultContainer.insertAdjacentHTML('beforeend', '<h3>No hay coincidencias</h3>');
                        return;
                  }
                   // si no esta vacío inyecto las cards
                  usersAndMovies.forEach(userAndMovie => {
                        const { username, movie, watched, img, rate } = userAndMovie;
                        const card = createCard({ username, movie, watched, img, rate });

                        // agrego la card
                        resultContainer.insertAdjacentHTML('beforeend', card);

                        //'scrolleo' hasta el contenedor con los resultados
...
//Función principal 
const filterMovies = ({ users, movies, userId = null, fromDate, toDate, rate, }) => {

      if (userId) {
            const moviesWithId = movies.filter((movie) => movie.userId === Number(userId));
            return filtersWithOutUserId({ users, moviesWithId, fromDate, toDate, rate });
      } else {
            return filtersWithOutUserId({ users, movies, fromDate, toDate, rate });
      }

};

//Función para filtrar las peliculas y los usuarios sin el valor del userId
const filtersWithOutUserId = ({ users, movies, fromDate, toDate, rate }) => {
      const fromDateSeconds = new Date(fromDate).getTime() / 1000;
      const toDateSeconds = new Date(toDate).getTime() / 1000;
      const usersMovies = [];

      const moviesFilter = movies
            .filter(
                  (movie) => {
                        const dateSeconds = new Date(movie.watched.split(" ")[0]).getTime() / 1000;
...
//Función para crear la tarjeta con los datos
const createCard = ({ username, movie, watched, img, rate }) => {
      const localDate = new Date(watched);
      ...
      const card = `
                  <div class="card">
                        ...
                        <div class="card-info">
                              ...
                              <p>Película: <span>${movie}</span></p>
                              <p>Vista el: <span>${day}/${month}/${year}</span></p>
...
//Función para limpiar el contenedor al volver a enviar el Form
const clearContainer = () => {
      resultContainer.innerHTML = "";
      if (resultTitle.classList.contains('results-title-off') && resultContainer.classList.contains('result-off')) {
            resultTitle.classList.remove('results-title-off');
            resultContainer.classList.remove('result-off');
      }

}

//Función simple para validar que los inputs no esten vacíos
const validateInput = () => {
